    ...
    $.ajax({
        ...
        success: function (jsn) {
            $.each(jsn.config, function (i)
            {
                ...
                configuracionesGson[id] = {nave: nav, difi: dif, luna: lun};
...
function cargarSeleccion() {
    var i = $("#configuracion option:selected").prop('id');
    construirPartida(configuracionesGson[i].nave, configuracionesGson[i].difi, configuracionesGson[i].luna);
...
function guardarConfi() {
    ...
    var nombre = $("#nomConf").val();
    var newId = $("#configuracion option").length;

    $.ajax({
        method: "POST",
        url: url,
        //enviamos los datos al post
        data: {id: newId, nm: nombre, d: dificultad, n: modeloNave, l: modeloLuna},
        success: function (u) {
                configuracionesGson[newId] = {nave: modeloNave, difi: dificultad, luna: modeloLuna};
            $("#nomConf").val(""); //reseteamos el nombre asi nos saldra el PlaceHolder.
            //añadimos la nueva opcion al select.
            $("#configuracion").append(
                    "<option id='" + newId + "'>" + nombre + " Dificultad: "
                    + dificultad + " Nave: " +
...
function construirPartida(nave, difi, luna) {
    l_nave = parseInt(nave);
    l_dificultad = parseInt(difi);
    l_luna = parseInt(luna);
    modeloNave=l_nave;
    modeloLuna=l_luna;
    dificultad=l_dificultad;
